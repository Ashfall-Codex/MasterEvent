...
public static class TemplateSyncHelper
{
    ...
    public static SheetSyncReport SyncSheetWithTemplate(PlayerSheet sheet, EventTemplate template)
    {
        ...
        return SyncStatsAndCounters(sheet.Stats, sheet.Counters, template);
    }

    public static SheetSyncReport SyncStatsAndCounters(
        List<StatValue> stats, List<CustomCounter> counters, EventTemplate template)
    {
        var statsAdded = 0;
        var statsRenamed = 0;
        ...
        var statsRemoved = stats.RemoveAll(s => !templateStatIds.Contains(s.Id));
        ...
        if (template.StatDefinitions != null)
        {
            ...
            {
                var existing = stats.FirstOrDefault(s => s.Id == def.Id);
                if (existing == null)
                {
                    stats.Add(def.ToStatValue());
                    statsAdded++;
                }
        ...
        var countersRemoved = counters.RemoveAll(c => !templateCounterIds.Contains(c.Id));

        if (template.CounterDefinitions != null)
        {
            foreach (var def in template.CounterDefinitions)
            {
                var existing = counters.FirstOrDefault(c => c.Id == def.Id);
                if (existing == null)
                {
                    counters.Add(def.ToCounter());
                    countersAdded++;
                }
                else if (existing.Name != def.Name)
                {
